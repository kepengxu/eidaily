...
async function isAINews(title, content) {
  // 共享 matcher：先识别两大领域（大模型 / 具身智能）。命中主题即视为相关，
  // 不再被旧黑名单或「具身智能」标题前缀规则拒绝（前缀排除已删除）。
  if (classifyText(`${title} ${content || ''}`)) {
    return true;
  }
  return await isAINewsWithAI(title, content);
}

// 统一两领域分类（大模型 / 具身智能），直接复用共享 matcher。
// 命中主题即视为相关并归入对应领域；未命中返回 null 不进入系统。
// 不再使用旧四分类（国内AI/国外AI/科技新闻/AI趣味新闻），以新数据为主。
...
async function processNewsItem(item, source) {
  ...
  const category = await getNewsCategory(item.title, content || item.content || '');
...
async function main() {
  ...
  try {
    ...
    if (parsedData.success && parsedData.data) {
      ...
      // 重新处理现有新闻的分类，并实际过滤掉非相关历史记录
      const reclassifiedExisting = [];
      for (const item of existingNews) {
        const newCategory = await getNewsCategory(item.title, item.content);

        // 如果不是AI相关新闻（含具身智能/大模型以外的历史记录），实际剔除
        if (!newCategory) {
          console.log(`过滤掉历史非相关新闻: ${item.title}`);
          continue;
        }
        ...
        reclassifiedExisting.push(item);
      }
      
      // 对现有新闻进行过滤和排序，然后添加到总列表
      const validExistingNews = reclassifiedExisting.filter(item => {
        // 应用相同的过滤规则到现有新闻
