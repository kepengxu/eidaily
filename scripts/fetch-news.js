...
async function fetchNews() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.articles && data.articles.length > 0) {
            ...
            console.log(`✅ NewsAPI.org (${domain}) 获取到 ${data.articles.length} 条新闻`);
          } else {
            console.log(`⚠️ NewsAPI.org (${domain}) 没有返回文章数据`);
          }
        } else {
          const errorData = await response.json();
          console.error(`❌ NewsAPI.org (${domain}) 请求失败: ${response.status} ${response.statusText}`, errorData);
        }
      } catch (error) {
        console.error(`❌ NewsAPI.org (${domain}) 连接错误:`, error.message);
      }
    }
  } else {
  ...
  // 备用：尝试NewsAPI.ai - 拆分为「大模型 / 具身智能」两个独立查询对象
  // 不再受 allNews.length < 30 阈值约束，避免某条新闻数量阈值导致具身智能被遗漏。
  if (newsApiKey) {
    for (const domain of ['llm', 'embodied']) {
      try {
        console.log(`正在尝试NewsAPI.ai (${domain})...`);
        const response = await fetch(
          `https://newsapi.ai/api/v1/article/getArticles`, {
            ...
            body: JSON.stringify({
              apiKey: newsApiKey,
              query: newsApiQuery(domain, 'en', fromDate),
              resultType: "articles",
              articlesSortBy: "date",
              articlesCount: 20
            })
        ...
        if (response.ok) {
          ...
          if (data.articles && data.articles.results && data.articles.results.length > 0) {
            ...
            console.log(`✅ NewsAPI.ai (${domain}) 获取到 ${formattedNews.length} 条新闻`);
          } else {
            console.log(`⚠️ NewsAPI.ai (${domain}) 没有返回文章数据`);
          }
        } else {
          const errorData = await response.json();
          console.error(`❌ NewsAPI.ai (${domain}) 请求失败: ${response.status} ${response.statusText}`, errorData);
        }
      } catch (error) {
        console.error(`❌ NewsAPI.ai (${domain}) 连接错误:`, error.message);
      }
    }
  } else {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.results && data.results.length > 0) {
            ...
            allNews = [...allNews, ...formattedNews];
            console.log(`✅ NewsData API (${domain}) 获取到 ${formattedNews.length} 条最新新闻`);
          } else {
            console.log(`⚠️ NewsData API (${domain}) 没有返回结果数据`);
          }
        } else {
          const errorData = await response.json();
          console.error(`❌ NewsData API (${domain}) 请求失败: ${response.status} ${response.statusText}`, errorData);
        }
      } catch (error) {
        console.error(`❌ NewsData API (${domain}) 连接错误:`, error.message);
      }
    }
  } else {
  ...
  // 尝试GNews - 获取最近的新闻
  // 复用共享两领域 query，按 llm / embodied 独立遍历（每服务最多 2 次）；
  // 保留原有备用数量阈值（allNews.length < 80），阈值内同一服务两领域都请求。
  if (gnewsApiKey && allNews.length < 80) {
    for (const domain of ['llm', 'embodied']) {
      try {
        const q = encodeURIComponent(serviceQuery(domain, 'en'));
        console.log(`正在尝试GNews API (${domain})...`);
        const response = await fetch(
          `https://gnews.io/api/v4/search?q=${q}&lang=en&country=us&max=50&from=${fromDate}T00:00:00Z&apikey=${gnewsApiKey}`
        );
        ...
        if (response.ok) {
          ...
          if (data.articles && data.articles.length > 0) {
            ...
            allNews = [...allNews, ...formattedNews];
            console.log(`✅ GNews (${domain}) 获取到 ${formattedNews.length} 条新闻`);
          } else {
            console.log(`⚠️ GNews (${domain}) 没有返回文章数据`);
          }
        } else {
          const errorData = await response.json();
          console.error(`❌ GNews (${domain}) 请求失败: ${response.status} ${response.statusText}`, errorData);
        }
      } catch (error) {
        console.error(`❌ GNews (${domain}) 连接错误:`, error.message);
      }
    }
  } else if (!gnewsApiKey) {
  ...
  // 尝试Currents API - 拆分为「大模型 / 具身智能」两个独立短关键词请求
  // 不再受 allNews.length < 100 阈值约束，避免具身智能被遗漏；关键词保持简短以控制费用。
  if (currentsApiKey) {
    for (const domain of ['llm', 'embodied']) {
      try {
        const keywords = encodeURIComponent(currentsKeywords(domain, 'en'));
        console.log(`正在尝试Currents API (${domain})...`);
        const response = await fetch(
          `https://api.currentsapi.services/v1/search?keywords=${keywords}&language=en&start_date=${fromDate}&apiKey=${currentsApiKey}`
        );
        ...
        if (response.ok) {
          ...
          if (data.news && data.news.length > 0) {
            ...
            allNews = [...allNews, ...formattedNews];
            console.log(`✅ Currents API (${domain}) 获取到 ${formattedNews.length} 条新闻`);
          } else {
            console.log(`⚠️ Currents API (${domain}) 没有返回新闻数据`);
          }
        } else {
          const errorData = await response.json();
          console.error(`❌ Currents API (${domain}) 请求失败: ${response.status} ${response.statusText}`, errorData);
        }
      } catch (error) {
...
// 两类新闻分类：大模型 / 具身智能
// 内部匹配委托给共享 matcher（scripts/news-matcher.js），保留函数接口不变；
// 原文回退逻辑见 categorizeNews（翻译可能丢失模型缩写，因此再检查英文原文）。
function categorizeNewsFocused(title, content) {
  return classifyText(`${title} ${content || ''}`);
}
...
async function categorizeNews(title, content, originalTitle = '', originalContent = '') {
  const translatedCategory = categorizeNewsFocused(title, content);
  if (translatedCategory) return translatedCategory;

  // 翻译可能丢失模型缩写，因此再检查英文原文。
  return categorizeNewsFocused(originalTitle, originalContent);
}
