...
const CACHE_PREFIX = `ai-news-${BASE_URL}-`;
const CACHE_NAME = `${CACHE_PREFIX}v1.0.3`;
const STATIC_CACHE = `${CACHE_PREFIX}static-v1.0.3`;
    ...
    caches.keys().then(cacheNames => {
      return Promise.all(
        cacheNames.map(cacheName => {
          if (cacheName.startsWith(CACHE_PREFIX) && cacheName !== CACHE_NAME && cacheName !== STATIC_CACHE) {
...
self.addEventListener('fetch', (event) => {
  ...
  if (request.mode === 'navigate') {
    event.respondWith(
      fetch(`${BASE_URL}index.html`).catch(() => caches.match(`${BASE_URL}index.html`))
    );
    return;
  }
  ...
  event.respondWith(
    caches.match(request).then((cached) => {
      return cached || fetch(request).catch(() => caches.match(`${BASE_URL}index.html`));
    })
  );
});
