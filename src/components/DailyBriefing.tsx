...
export const DailyBriefing: React.FC<DailyBriefingProps> = ({ isOpen, onClose }) => {
  ...
  const { getLocalizedNewsArray } = useNewsTranslation();
  ...
  const generateBriefing = async () => {
    ...
    try {
      // 获取AI相关新闻并生成晨报
      const response = await fetch(`${import.meta.env.BASE_URL}news-data.json?` + Date.now());
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="bg-gradient-hero text-white p-6 relative">
          ...
          <div className="flex items-center space-x-2 text-sm opacity-90">
            ...
            <span>{isZh ? '实时AI新闻' : 'Real-time AI News'}</span>
          </div>
        </div>
        ...
        <div className="border-t border-gray-200 p-4 bg-gray-50">
          <div className="text-center">
            <p className="text-sm text-gray-500">
              {isZh 
                ? 'AI专题简报基于站内实时新闻数据生成，持续关注人工智能领域最新动态'
                : 'AI topic briefing is generated based on real-time news data, continuously tracking the latest developments in artificial intelligence'
              }
            </p>
          </div>
        </div>
